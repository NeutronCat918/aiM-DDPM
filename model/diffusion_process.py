...
class DiffusionModel(nn.Module):
    ...
    def denoise_loop(self,anom_x:torch.Tensor,return_all_timesteps: bool = False)->torch.Tensor:
        print("denoising...")
        timestamp = torch.randint(self.num_timesteps-5, self.num_timesteps, (1,)).long().to(self.device)
        anom_x_norm = self.normalize(anom_x)
        noise = torch.randn_like(anom_x_norm)
        anom_noised=self.q_sample(anom_x_norm, timestamp, noise=noise)
        predicted_noise = self.model(anom_noised, timestamp)
       
        ret = anom_noised-predicted_noise
    ...
    def sample(
        self, batch_size: int = 16, return_all_timesteps: bool = False, initial_image:torch.Tensor = None
    ) -> torch.Tensor:
        print("sampling called")
        shape = (batch_size, self.channels, self.image_size, self.image_size)
        if initial_image != None:
            return self.denoise_loop(initial_image, return_all_timesteps=return_all_timesteps)
        else:
